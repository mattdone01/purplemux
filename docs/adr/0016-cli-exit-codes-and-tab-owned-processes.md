...
5. `tab send --wait-ms` is capped at 290000 in the CLI. Node's `fetch` stops waiting for response headers at 300 s, so a longer wait would end in `outcome-unknown` while the server could still paste.
