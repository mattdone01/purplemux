...
- Boot: the workspace store's cross-check adopts an orphan session under the tab id its token was minted for (exact workspace and session name), so the layout id equals the shell's `PMUX_TAB_ID`; a pre-token orphan gets a new id. `initTabTokens` then removes, logs and emits `tab-closed` (reason `boot-sweep`) for every token whose tab no longer exists, and moves any token whose exact session name another tab id now holds. The server never parses a tab id out of a session name.
- A new token that cannot be saved is withdrawn and the tab launches without tab identity; an unreadable token file is moved aside, never overwritten. A workspace delete revokes every token of the workspace, including tabs that never reached its layout.
