...
- Boot (`initTabTokens`, after the workspace store's cross-check): a token whose tab no longer exists is removed, logged, and emits `tab-closed` with reason `boot-sweep`. A token whose exact session name is now held by an adopted orphan moves to the adopted tab id. The server never parses a tab id out of a session name.
