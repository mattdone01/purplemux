...
// times and counts, an epic slug the sender holds, and — for a watch, which
// goes only to the tab that registered it — that tab's own validated target.
...
const GRAMMAR = {
  ...
  missionId: /^[A-Za-z0-9][A-Za-z0-9._:-]{0,79}$/,
...
export interface INoteFields { noteId: string; fromWorkspaceId: string | null; fromTabId: string | null; sentAt: number; epic?: string | null }
...
export interface IMissionFields { itemId: string; workspaceId: string; readyAt: number }
...
const TEMPLATES: { [K in TInboxKind]: TRenderer<K> } = {
  note: (f) => {
    ...
    const epic = f.epic == null ? '' : ` for epic ${field('epic', f.epic, 'epic')}`;
    return { recordId: id, line: `[purplemux note ${id}] from ${from}${epic} at ${time('sentAt', f.sentAt)} — purplemux note show ${id}` };
  },
  watch: (f) => {
  ...
  mission: (f) => {
    const id = field('itemId', f.itemId, 'missionId');
